...
echo "---"
check_port_8080
echo "Section,Run,RSS,Time,Startup,TimeToReady" > "$OUTPUT_CSV"
...
for run in $(seq 1 $NUM_RUNS); do
  for section in "${SECTIONS_LIST[@]}"; do
    ...
    TIME_TO_READY=$(echo "$result" | tail -n 1 | cut -d',' -f4)
    echo "$section,$run,$RSS,$TIME,$STARTUP,$TIME_TO_READY" >> "$OUTPUT_CSV"
    echo "  --> RSS=$RSS Time=$TIME Startup=$STARTUP TimeToReady=$TIME_TO_READY"
  done
  sleep 2
done
...
for section in "${SECTIONS_LIST[@]}"; do
  ...
  avg_time=$(grep "^$section," "$OUTPUT_CSV" | cut -d',' -f4 | awk '{sum+=$1} END {printf "%.2f", sum/NR}')
  avg_startup=$(grep "^$section," "$OUTPUT_CSV" | cut -d',' -f5 | awk '{sum+=$1} END {printf "%.3f", sum/NR}')
  avg_ready=$(grep "^$section," "$OUTPUT_CSV" | cut -d',' -f6 | awk '{sum+=$1} END {printf "%.3f", sum/NR}')
  echo "$section,avg,$avg_rss,$avg_time,$avg_startup,$avg_ready" >> "$OUTPUT_CSV"
  echo "Avg $section: RSS=$avg_rss Time=$avg_time Startup=$avg_startup TimeToReady=$avg_ready"
done
